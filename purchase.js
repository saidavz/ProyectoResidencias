...
// ---- BUSCADOR DE PROYECTOS (AUTOCOMPLETE) ----

const projectSearchInput = document.querySelector('.project-input');
const projectSuggestions = document.querySelector('.sugerencias-project');
...
projectSearchInput.addEventListener('input', async () => {
  ...
  try {
    const res = await fetch(`${baseURL}/projects/active`);
    ...
    const ordered = filtered.sort((a, b) => {
      const an = normalizeText(a.name_project || '');
      const bn = normalizeText(b.name_project || '');
    ...
    ordered.forEach(p => {
      ...
      div.textContent = `${p.no_project} - ${name}`;
      div.style.padding = '8px';
      div.style.cursor = 'pointer';
      div.style.borderBottom = '1px solid #555';

      div.addEventListener('click', () => {
        projectSearchInput.value = `${p.no_project} - ${name}`;
        projectSelectHidden.value = p.no_project; // guardar valor real
        projectSuggestions.style.display = 'none';
      });
    ...
    projectSuggestions.style.display = ordered.length ? 'block' : 'none';
...
document.addEventListener('click', (e) => {
  if (!projectSuggestions.contains(e.target) && e.target !== projectSearchInput) {
    projectSuggestions.style.display = 'none';
  }
});
async function setupAutocomplete(inputSelector, sugerenciasSelector, hiddenSelector, url, keyField, textField) {
  const input = document.querySelector(inputSelector);
  const box = document.querySelector(sugerenciasSelector);
  ...
  input.addEventListener("input", async () => {
    ...
    try {
      ...
      const filtered = data.filter(item => {
        const txt = normalize(item[textField] || "");
        const key = normalize((item[keyField] || "").toString());
        return txt.includes(term) || key.includes(term);
      });

      const sorted = filtered.sort((a, b) => 
        normalize(a[textField]).localeCompare(normalize(b[textField]))
      );

      box.innerHTML = "";
      sorted.forEach(item => {
        const div = document.createElement("div");
        div.textContent = `${item[keyField]} - ${item[textField]}`;
        div.style.padding = "8px";
        div.style.cursor = "pointer";
        div.style.borderBottom = "1px solid #555";

        div.addEventListener("click", () => {
          input.value = `${item[keyField]} - ${item[textField]}`;
          hidden.value = item[keyField];
          box.style.display = "none";
        });
